...
By default, Relay does not intercept `/`, `Escape`, `Ctrl+C`, or ordinary text. `Ctrl+Q` is the default primary switch, but the user may replace it with any terminal-observable single key chord. `F6` is the fixed fallback. Relay has no prefix chord or selector.

The input router compiles the configured OpenCode-style key name into every applicable legacy terminal sequence and also matches CSI-u/Kitty enhanced keyboard encodings and xterm's modified-key form. This allows arbitrary modifier combinations, named functional keys, Unicode characters, and raw enhanced key codes without a Relay-owned allowlist. Some chords remain dependent on the outer terminal reporting them distinctly. The router consumes only a key press, not a repeat or release. Relay also understands bracketed-paste boundaries, including markers split across input chunks, so pasted shortcut bytes cannot accidentally switch harnesses. Every other key sequence is forwarded to the native application.
...
```text
~/.local/share/relay/
  config.json                # versioned user settings, including switch key
...
- Every managed Relay-owned child has a private, secret-free ownership record with its recovery scope. After SIGKILL, the next operational Relay launch compares both owner and child OS start identities before terminating an orphaned process or process group; an identity mismatch is discarded without signaling the live process. Failed claims remain available for a later safe retry and report their recorded identity and scope. Static help and version output do not open Relay storage or run orphan recovery.
...
- A definitively deleted vendor session is replaced once with a cold session and the bounded canonical handoff. A context-limit failure preserves the existing binding so the user can compact that same session. If a warm headless command fails after the vendor session may have advanced, Relay retires that uncertain binding rather than risk duplicating the prompt on retry. The error explains that the native session remains in vendor history and that retrying creates a fresh session from confirmed Relay context.
